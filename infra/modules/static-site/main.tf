...
# ---------- Origin bucket (private; CloudFront-only access via OAC) ----------
...
# Rewrites clean URLs to the index documents emitted by `next build` with
# trailingSlash: true (e.g. /resume -> /resume/index.html).
...
resource "aws_cloudfront_distribution" "site" {
  ...
  default_cache_behavior {
    ...
    # AWS managed policies: CachingOptimized + (no origin request policy needed for S3)
